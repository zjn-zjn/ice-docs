...
- Server：修复 `ImportData` 仅导入 Confs（无 Bases）时未触发版本号递增的问题
- Server：`/conf/edit` 单节点创建（editType=1/4）也返回完整节点数据
- Web UI：修复新增子节点后双击属性表单为空、需刷新页面才能正常显示的问题

仅 Server 更新，SDK 无变更，替换 Server 即可。
...
- Web UI：节点工具栏新增对比按钮，应用/清除确认弹框展示变更节点列表
- Web UI：新增全屏双树对比视图（已发布树 vs 当前编辑树），变更节点高亮
...
- 修复导入数据时未更新 ID 计数器的问题，建议尽快升级 Server
- SDK 无变更，无需修改代码
