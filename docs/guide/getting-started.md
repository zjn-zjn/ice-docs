...
| 平台 | 下载包 |
|------|--------|
| Linux x86_64 | `ice-server-linux-amd64.tar.gz` |
| Linux ARM64 | `ice-server-linux-arm64.tar.gz` |
| macOS x86_64 | `ice-server-darwin-amd64.tar.gz` |
| macOS ARM64 (Apple Silicon) | `ice-server-darwin-arm64.tar.gz` |
| Windows x86_64 | `ice-server-windows-amd64.tar.gz` |

```bash
# 解压并启动（Linux/macOS）
tar -xzvf ice-server-linux-amd64.tar.gz
cd ice-server-linux-amd64
sh ice.sh start
```
